...
class SigningRequestManager {
  ...
  List<String?> getRequiredAbis() {
    ...
    rawActions.removeWhere((ESRAction action) => SigningRequestUtils.isIdentity(action));
  ...
  List<ESRAction> resolveActions(Map<String?, dynamic> abis, Authorization signer) {
  ...
  List<ESRAction> getRawActions() {
    final req = signingRequest.req;
    switch (req[0]) {
      case 'action':
        return [ESRAction.fromJson(Map<String, dynamic>.from(req[1]))];
      case 'action[]':
      ...
      case 'transaction':
        ...
        final List<ESRAction> actions = [];
        for (final item in actionsRaw) {
          actions.add(ESRAction.fromJson(Map<String, dynamic>.from(item)));
        }
        return actions;
      default:
        throw 'Invalid signing request data';
...
class SigningRequestUtils {
  ...
  static Future<void> serializeActions(List<ESRAction?> actions, {AbiProvider? abiProvider}) async {
  ...
  static ESRAction deserializeAction(int version, eosDart.Contract contract, String account, String? name,
  ...
  static bool isIdentity(ESRAction? action) {
