...
typedef SuccessCallback = Future<void> Function(String baseURI);
typedef FailureCallback = Future<void> Function(String errorMessage, String baseUrl);

class TransactionCallbackInterface {
  final SuccessCallback? success;
  final FailureCallback? failure;

  TransactionCallbackInterface({this.success, this.failure});
}
...
//    print(abiResp.abi);
  ...
  Future<dio.Response> pushTransaction(
    ...
    TransactionCallbackInterface? callback,
  }) async {
    var attempt = 0;
    ...
    while (attempt < maxRetries) {
      try {
        ...
        await callback?.success?.call(options.baseUrl);
        return processedTrx;
      } catch (e) {
        final errorString = e.extendedBlockchainErrorMessage().toLowerCase();
        const String CPU_LIMIT_ERROR = 'exceeded the current cpu usage limit';
        const String EXECUTION_TIMEOUT_ERROR = 'executing for too long';
        final isTimeoutError = errorString.contains(CPU_LIMIT_ERROR) || errorString.contains(EXECUTION_TIMEOUT_ERROR);
        if (isTimeoutError) {
          timeoutException = e;
          attempt++;
          print('Timeout error occurred. Retrying attempt $attempt/$maxRetries after $retryDelay...');
          await Future.delayed(retryDelay);
          await callback?.failure?.call(options.baseUrl, errorString);
        } else {
          LogHelper.e('Unhandled exception in push transaction: ${e.toString()}');
          rethrow;
        }
      }
    }
